...
- `votes_count`: Numeric (Default: 0) _(Note: Previously attempted as `upvotes`/`downvotes`, but migrated to a single `votes_count` metric to prevent 500 errors)._
...
_(Note: Implementation for unique user voting tracking)_
