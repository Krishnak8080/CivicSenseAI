...
export const ReportCard = React.forwardRef<HTMLDivElement, ReportCardProps>(
  ({ report, onUpvote, onDownvote, voteType = 0 }, ref) => {
    const priorityColors = {
      high: 'bg-[var(--priority-high)]/10 text-[var(--priority-high)] border-[var(--priority-high)]/30',
      medium:
        'bg-[var(--priority-medium)]/10 text-[var(--priority-medium)] border-[var(--priority-medium)]/30',
        ...
        <Card className="bg-[var(--bg-card)] border-[var(--border-subtle)] overflow-hidden transition-all duration-300 hover:border-[var(--orange-primary)]/50 hover:shadow-glow group">
          <CardHeader className="pb-3 px-5 pt-5">
            <div className="flex justify-between items-start">
              ...
              <div className="flex flex-col items-end space-y-2">
                <Badge
                  variant="outline"
                  className={`px-2.5 py-0.5 text-xs font-semibold ${priorityColors[report.priority]}`}
                >
                ...
                {report.status === 'resolved' && (
                  <Badge
                    variant="outline"
                    className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30 px-2.5 py-0.5 text-xs font-semibold"
                  >
                    RESOLVED
                  </Badge>
                )}
          ...
          <CardFooter className="flex justify-between items-center px-5 py-4 bg-[var(--bg-elevated)]/50 border-t border-[var(--border-subtle)]">
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                <ArrowUpCircle
                  className={`w-6 h-6 ${voteType === 1 ? 'fill-[var(--orange-primary)]/20 text-[var(--orange-primary)]' : ''}`}
                />
              ...
              >
                <ArrowDownCircle
                  className={`w-6 h-6 ${voteType === -1 ? 'fill-blue-500/20 text-blue-400' : ''}`}
                />
              </Button>
            </div>
          </CardFooter>
