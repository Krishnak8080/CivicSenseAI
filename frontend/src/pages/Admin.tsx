...
const StatCard = ({ title, value, icon: Icon, trend, color }: any) => (
  <Card className="bg-[var(--bg-card)] border-[var(--border-subtle)] p-6 flex flex-col justify-between hover:border-[var(--orange-primary)]/30 transition-colors">
    <div className="flex justify-between items-start mb-4">
      ...
      <Badge
        variant="outline"
        className="bg-[var(--bg-elevated)] border-[var(--border-subtle)] text-[var(--text-secondary)]"
      >
        {trend}
      </Badge>
    </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-[var(--text-secondary)] mt-1">
              Overview of all civic issues and platform activity.
            </p>
          </div>
        </div>
            ...
            <Table>
              <TableHeader className="bg-[var(--bg-elevated)] border-b border-[var(--border-subtle)]">
                <TableRow className="hover:bg-transparent border-none">
                  <TableHead className="text-[var(--text-muted)] font-semibold h-12">
                    Title
                  </TableHead>
                  <TableHead className="text-[var(--text-muted)] font-semibold h-12">
                    Location
                  ...
                  <TableHead className="text-right text-[var(--text-muted)] font-semibold h-12">
                    Action
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                <AnimatePresence mode="popLayout">
                  {reports.map((report) => (
                    ...
                    >
                      ...
                      <TableCell className="text-[var(--text-secondary)] truncate max-w-[200px]">
                        {report.location}
                      </TableCell>
                      <TableCell>
                        <Badge
                ...
                {reports.length === 0 && (
                  <TableRow>
                    <TableCell
                      colSpan={6}
                      className="text-center h-32 text-[var(--text-muted)] border-none"
                    >
